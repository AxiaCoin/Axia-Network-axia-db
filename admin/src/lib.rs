...
//! Experimental admin functionality for axia-db.
...
pub fn run() -> Result<(), String> {
	...
	let mut options = if let Some(metadata) = axia_db::Options::load_metadata(&metadata_path)
		.map_err(|e| format!("Error resolving metas: {:?}", e))? {
		let mut options = axia_db::Options::with_columns(db_path.as_path(), 0);
		options.columns = metadata.columns;
		options.salt = Some(metadata.salt);
		options
	} else {
		axia_db::Options::with_columns(db_path.as_path(), nb_column)
	};
	...
	match cli.subcommand {
		SubCommand::Stats(stat) => {
			let db = axia_db::Db::open_read_only(&options)
		...
		SubCommand::Migrate(args) => {
			...
			axia_db::migrate(&db_path, dest_options, args.overwrite, &args.force_columns)
		...
		SubCommand::Check(check) => {
			...
			let check_param = axia_db::CheckOptions::new(
				check.column,
				check.range_start,
				check.range_end,
				check.display,
		...
		SubCommand::Flush(_flush) => {
			let _db = axia_db::Db::open(&options)
				.map_err(|e| format!("Invalid db: {:?}", e))?;
		},
